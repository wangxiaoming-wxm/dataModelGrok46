...
- 客户侧：车龄暴露、车况差更愿索赔、新车+好车况几乎不报、高龄右尾、地区异质。显式 `claim_util = rate * (1-deny)`。
- 保险公司侧：先按 source 分层核保；condition 形状随车型变；质保窗少赔/不赔。**融合后冻结硬门**（`InsurerGate`），不要再网格搜平移量。
- 对抗 Ridge 与 teacher 相关 0.80，融了掉分。细节：`analysis/ADVERSARIAL.md`。
