...
- Insurer does not pay in warranty/franchise pits:
    days in [1725, 1825)  — train n=103, 0 claims (confirm p=0.006); ~5*365
    days in [700, 880)    — train rate 3.07% vs 10.02%; ~2*365
    days in [2110, 2210)  — train n=142, rate 2.8%; ~6*365 (nested 10/10 floor)
- End-of-cover dump: days in [9370, 9475) rate 18.9%

CatBoost still ranks the all-zero 5y pit too high (mean rank ~0.22),
and the 6y pit mean rank ~0.32. Apply AFTER rank fusion.
Magnitudes frozen (not nested-tuned except 6y pre-registered as floor):
  zero 1750 and 2110;  rank -0.10 on 750;  rank +0.05 on hot.
...
def attach_adv_columns(df: pd.DataFrame, cond_rk: np.ndarray | None = None) -> pd.DataFrame:
    ...
    deny = _mask(days, *W750) | _mask(days, *W1750) | _mask(days, *W2110)
    ...
    out["w_safe2110"] = _mask(days, *W2110).astype(np.float64)
